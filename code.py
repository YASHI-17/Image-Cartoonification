...
def cartoonify(ImagePath):
    # read the image
    originalmage = cv.imread(ImagePath)
    originalmage = cv.cvtColor(originalmage, cv.COLOR_BGR2RGB)
    ...
    ReSized1 = cv.resize(originalmage, (960, 540))
    # plt.imshow(ReSized1, cmap='gray')

    # converting an image to grayscale
    grayScaleImage = cv.cvtColor(originalmage, cv.COLOR_BGR2GRAY)
    ReSized2 = cv.resize(grayScaleImage, (960, 540))
    # plt.imshow(ReSized2, cmap='gray')

    # applying median blur to smoothen an image
    smoothGrayScale = cv.medianBlur(grayScaleImage, 5)
    ReSized3 = cv.resize(smoothGrayScale, (960, 540))
